...
def test_version_is_exported() -> None:
    ...
    assert re.fullmatch(r"\d+\.\d+\.\d+", moneyness.__version__)
...
def test_exported_version_matches_the_installed_distribution() -> None:
    ...
    version is a lie.
    """
    assert moneyness.__version__ == version("moneyness")
...
def test_exported_version_matches_the_source_of_truth() -> None:
    ...
    assert moneyness.__version__ == declared
...
def test_licence_reaches_the_built_distribution() -> None:
    ...
    `license-files` in pyproject.toml is what does this. Dropping it leaves the
    metadata still claiming MIT while the wheel ships no terms at all, which is
    the state this project shipped in before.
    """
    fields = metadata("moneyness")
...
def test_the_typing_marker_ships() -> None:
    ...
    """
    assert (Path(moneyness.__file__).parent / "py.typed").is_file()
    assert "Typing :: Typed" in metadata("moneyness").get_all("Classifier", [])


def test_the_entry_point_reports_the_version() -> None:
    """`moneyness --version` prints it, and exits zero doing so.
